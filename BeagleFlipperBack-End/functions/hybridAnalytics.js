...
async function getHybridSuggestion(userState, db, displayName, timeframe = 5) {
    ...
    // (PRIORITY 1 & 2 are unchanged from your file)
    ...
    if (cashPerSlot < config.TRADING_CONFIG.MIN_CASH_PER_SLOT) return { type: "wait", message: "Not enough cash per slot." };
    ...
    const commodityCheckPromises = Object.keys(config.TARGET_COMMODITIES).map(async (itemIdStr) => {
        ...
        if (blockedItemsSet.has(itemId) || recentlySuggestedSet.has(itemId)) {
            return null;
        }
